...
function Map() {
  ...
  const fetchWeatherCoords = useWeatherStore(
    (state) => state.fetchWeatherCoords
  );
  ...
  return (
    <div className="h-screen flex flex-col md:grid md:grid-cols-[70px_1fr]">
      ...
      {userLocation ? (
        <MapContainer
          className="w-full h-full md:rounded-r-lg md:rounded-l-none"
          center={userLocation}
          zoom={13}
        >
          <TileLayer
              ...
              {weatherData ? (
                <div className="p-3 rounded-lg shadow-md bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 w-40">
                  ...
                  <p className="text-sm text-center italic mt-1">
                    {weatherData.currentData.description}
                  </p>
                </div>
              ) : (
